...
struct NewsHelicopterTests {
	...
	func reportBuildsAndStores() throws {
		...
		#expect(report.images.count < Self.builderImages().count, "only the images frames land in ride along")
		#expect(report.images.contains { $0.name == report.app.executable }, "plus the executable")
		#expect(report.crumbs?.screen == "Report test")
		#expect(report.elapsedMilliseconds > 0)
		#expect((report.memoryFootprintBytes ?? 0) > 0)
